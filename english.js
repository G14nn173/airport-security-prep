...
function render(){
  ...
  el.options.innerHTML=q.options.map((o,i)=>{
    ...
    return `<button class="${cls}" data-i="${i}" ${given!=null?"disabled":""}><strong>${LETTERS[i]})</strong> ${o}</button>`;
  }).join("");
  el.options.querySelectorAll("button").forEach(b=>b.addEventListener("click",()=>{if(state.answers[state.current]!=null)return;state.answers[state.current]=Number(b.dataset.i);render();}));
  if(given!=null){el.feedback.textContent=q.explanation;el.feedback.classList.remove("hidden");}
  else el.feedback.classList.add("hidden");
  el.prev.disabled=true;
  el.next.disabled=given==null;
  el.finish.disabled=given!=null;
...
el.next.addEventListener("click",()=>{if(state.answers[state.current]==null)return;if(state.current<state.questions.length-1){state.current++;render();}else finish(false);});
